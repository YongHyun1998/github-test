...
function MakeSiDoGun(target) {
	...
	// }
	//for (var i = 0; i < data.length; i++) {
		// var doGun = Object.values(data[2])[0];
		// for (var j = 0; j < doGun.length; j++) {
		// 	console.log(doGun[j]);
		// }
	//}
	var korea = document.querySelector(target);
	var selSiDo = document.createElement("select");
	var selDoGun = document.createElement("select");
	for(var i = 0; i < data.length; i++) {
		selSiDo.add(new Option(Object.keys(data[i])[0], i));
	}
	var doGun = Object.values(data[0])[0];
	for (var j = 0; j < doGun.length; j++) {
		selDoGun.add(new Option(doGun[j], j));
	}
	selSiDo.onchange = function(e) {
		//console.log(selDoGun.hasChildNodes());
		//selDoGun.innerHTML = "";
		while (selDoGun.hasChildNodes()) {
			selDoGun.removeChild(selDoGun.firstChild);
		}
		// console.log(e.target.selectedIndex);
		doGun = Object.values(data[e.target.selectedIndex])[0];
		for (var j = 0; j < doGun.length; j++) {
			selDoGun.add(new Option(doGun[j], j));
		}
	}
	korea.appendChild(selSiDo);
	korea.appendChild(selDoGun);
}